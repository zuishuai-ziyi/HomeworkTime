（本项目使用 GLM 5.3, DeepSeek-V4, Minimax-M3 等模型构建）
...
- 远程全量更新：心跳发现新版本自动后台下载，到管理端指定生效时间（避开晚自习）自动替换重启，支持延迟更新与失败回滚
...
- 更新管理（上传全量更新包 zip 并发布、指定生效时间、改期；仅保留最新一个包，回滚 = 重传旧包）
- 一键安装（上传客户端整包 zip 生成 PowerShell 一键安装命令，目标机执行即自动下载解压并写好连接配置；支持接自托管 Sink 短链服务生成更短的安装命令）
- 用户管理（增删改查；不能删除自己 / 最后一个用户）
- 设备监控（在线判定、心跳时间、教室名改名、待更新版本）
...
└─ server/                         # Node.js 服务端 + Vue3 后台
   ...
   ├─ uploads/audio/               # 后台上传音频落点（另有 updates/、installs/ 运行时创建）
